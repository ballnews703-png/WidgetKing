...
// title, the channel image title, and lives on in old caches, so filter it
// on EVERY path rather than trusting position.
function cleanHeadlines(titles) {
  return titles.filter(function (t) { return String(t).trim().toLowerCase() !== "google news"; });
}
  ...
  try {
    if (fm.fileExists(cachePath)) {
      ...
      if (cached && (Date.now() - cached.at) < 30 * 60 * 1000 && Array.isArray(cached.titles)) {
        return cleanHeadlines(cached.titles).slice(0, count);
      }
    }
  } catch (e) {}
  try {
    ...
    const clean = cleanHeadlines(titles);
    titles.length = 0;
    for (const t of clean) titles.push(t);
